...
def test_download_to_directory_success(s3_setup: S3Env) -> None:
    ...
    bucket.put_object(Key=f"meta/singles/{env.artifact_base_name}", Body=b"metadata")
    bucket.put_object(Key=f"objects/{env.artifact_base_name}/file1", Body=b"data1")
    bucket.put_object(Key=f"objects/{env.artifact_base_name}/file2", Body=b"data2")
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        artifacts.download_to_directory(env.artifact_base_name, outdir)

        # Assert
        assert (
            outdir / f"{env.artifact_base_name}.s3_metadata.yaml"
...
def test_download_to_directory_invalid_path(s3_setup: S3Env) -> None:
    ...
    with pytest.raises(RuntimeError):
        artifacts.download_to_directory(
            env.artifact_base_name, "/invalid/path/does/not/exist"
        )
...
def test_download_to_directory_non_pathlike_raises(s3_setup: S3Env) -> None:
    ...
    artifacts = S3Artifacts(env.bucket_name)
    with pytest.raises(RuntimeError):
        artifacts.download_to_directory(env.artifact_base_name, "nopath")
...
def test_download_to_directory_no_metadata_raises(s3_setup: S3Env) -> None:
    ...
    with TemporaryDirectory() as tmpdir:
        with pytest.raises(IndexError):
            artifacts.download_to_directory(env.artifact_base_name, tmpdir)
...
def test_upload_from_directory_success(s3_setup: S3Env) -> None:
    ...
    for filename in [
        f"{env.artifact_base_name}-file1",
        f"{env.artifact_base_name}-file2",
        "container",
    ]:
    ...
    # Act
    artifacts = S3Artifacts(env.bucket_name)
    artifacts.upload_from_directory(env.artifact_base_name, env.tmp_path)

    # Assert
    bucket = env.s3.Bucket(env.bucket_name)
    keys = [obj.key for obj in bucket.objects.all()]
    assert f"objects/{env.artifact_base_name}/{env.artifact_base_name}-file1" in keys
    assert f"objects/{env.artifact_base_name}/{env.artifact_base_name}-file2" in keys
    assert f"meta/singles/{env.artifact_base_name}" in keys

    meta_obj = list(
        bucket.objects.filter(Prefix=f"meta/singles/{env.artifact_base_name}")
    )[0]
    ...
    raw_tags_response = env.s3.meta.client.get_object_tagging(
        ...
        Key=f"objects/{env.artifact_base_name}/{env.artifact_base_name}-file1",
    )
...
def test_upload_from_directory_with_delete(s3_setup: S3Env) -> None:
    ...
    release = env.tmp_path / f"{env.artifact_base_name}.release"
    release.write_text(RELEASE_DATA)

    artifact = env.tmp_path / f"{env.artifact_base_name}.kernel"
    artifact.write_bytes(b"fake")

    # Arrange: put dummy existing objects to be deleted
    bucket.put_object(
        Key=f"objects/{env.artifact_base_name}/{artifact.name}", Body=b"old data"
    )
    bucket.put_object(
        Key=f"meta/singles/{env.artifact_base_name}", Body=b"old metadata"
    )

    artifacts = S3Artifacts(env.bucket_name)

    # Act
    artifacts.upload_from_directory(
        env.artifact_base_name, env.tmp_path, delete_before_push=True
    )
    ...
    assert f"objects/{env.artifact_base_name}/{artifact.name}" in keys
    assert f"meta/singles/{env.artifact_base_name}" in keys
...
def test_upload_from_directory_invalid_dir_raises(s3_setup: S3Env) -> None:
    ...
    with pytest.raises(RuntimeError, match="invalid"):
        artifacts.upload_from_directory(env.artifact_base_name, "/invalid/path")
...
def test_upload_from_directory_invalid_artifact_name(s3_setup: S3Env) -> None:
    ...
    # Arrange
    env = s3_setup
    (env.tmp_path / f"{env.artifact_base_name}.release").write_text(RELEASE_DATA)
    ...
    assert (
        len(
            list(bucket.objects.filter(Prefix=f"meta/singles/{env.artifact_base_name}"))
        )
        == 1
    )
...
def test_upload_from_directory_with_platform_variant(s3_setup: S3Env) -> None:
    ...
    meta_obj = next(
        ...
        if o.key == f"meta/singles/{env.artifact_base_name}"
    )
    metadata = yaml.safe_load(meta_obj.get()["Body"].read())
...
def test_upload_directory_with_requirements_override(s3_setup: S3Env) -> None:
    ...
    env = s3_setup
    (env.tmp_path / f"{env.artifact_base_name}.release").write_text(RELEASE_DATA)
    (env.tmp_path / f"{env.artifact_base_name}.requirements").write_text(
        "uefi = false\nsecureboot = true\n"
    )
    artifact_file = env.tmp_path / f"{env.artifact_base_name}-artifact"
