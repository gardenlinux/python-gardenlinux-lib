...
def add_container_to_index(
    index: str, container: str, tag: str, insecure: bool, additional_tag: List[str]
) -> None:
    """
    ...
    index_container = Container(index, insecure=insecure)

    image_index = index_container.read_or_generate_index()
    image_index.append_manifest(manifest)

    index_container.push_index(image_index)
    index_container.push_index_for_tags(image_index, additional_tag)
...
def new_index(index: str, insecure: bool, additional_tag: List[str]) -> None:
    """
    ...
    index_container = Container(index, insecure=insecure)
    image_index = index_container.generate_index()
    index_container.push_index(image_index)
    index_container.push_index_for_tags(image_index, additional_tag)
